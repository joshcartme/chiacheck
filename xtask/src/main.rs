...
#[derive(Parser)]
#[command(name = "xtask", about = "Chiacheck workspace tasks")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
#[derive(Subcommand)]
enum Command {
    /// Generate `chiacheck/src/metrics/ast_type_map.rs` from the resolved `oxc_ast` crate.
    GenAstTypeMap,
    ...
    CheckOxcVersion,
    /// Build chiacheck in release mode and benchmark it against the main branch.
    ///
    /// Runs `chiacheck score` in TARGET_DIR the given number of times for both the
    /// current branch and the `main` branch (via a temporary git worktree), then
    /// prints per-run timings and averages for comparison.
    Bench {
        /// Directory to run `chiacheck score` in (must contain a `chiacheck.toml`).
        target_dir: PathBuf,
