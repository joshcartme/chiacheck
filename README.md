...
- **`cargo xtask bench <TARGET_DIR> <RUNS>`**:
    ...
    2. Times fiber score N times in TARGET_DIR using /usr/bin/time -f "ELAPSED:%e" (stdout/stderr from fiber suppressed)
    3. Creates a temporary git worktree for main under target/\_bench_main_worktree, builds it with a separate target dir (target/\_bench_main_target)
